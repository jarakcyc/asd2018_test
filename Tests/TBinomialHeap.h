...
public:
    ...
    Key change(Pointer& ptr, Key key);
...
template<class Key>
vector<int> TBinomialHeap<Key>::get_roots() {
    vector<int> res;
    Node* ver = root;
    while (ver != nullptr) {
...
template<class Key>
Key TBinomialHeap<Key>::erase(Pointer& pointer) {
...
template<class Key>
Key TBinomialHeap<Key>::change(Pointer& pointer, Key key) {
    Key res = erase(pointer);
    pointer = insert(key);
    return res;
}
