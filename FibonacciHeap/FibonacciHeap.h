...
public:
    ...
    void decrease(Pointer pointer, Key key);
...
template<class Key>
void FibonacciHeap<Key>::decrease(Pointer pointer, Key key) {
...
template<class Key>
void FibonacciHeap<Key>::erase(Pointer& pointer) {
    if (pointer.ptr->link_node == nullptr) {
        throw std::out_of_range("no such element");
    }

    decrease(pointer, min_node->value);
